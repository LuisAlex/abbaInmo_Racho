...
class PropertiesControllerLocalities extends PropertiesController
{	 
...
function publish()
	...
	$this->publish	= ( $this->getTask() == 'publish' ? 1 : 0 );		
		...
		$query = 'UPDATE #__properties_locality'
		...
		$link = 'index.php?option=com_properties&view=localities';
	...
	$db			=& JFactory::getDBO();
	$row		=& JTable::getInstance('locality','Table');
	$row->load( $uid );
	$row->move( $inc );			
	...
	function edit()
	{	
		JRequest::setVar( 'view', 'localities' );
...
$msg = 	JText::_('Saved').' ( '.$post['name'].' ) ';
		...
		{
			case 'apply':
	$this->setRedirect( 'index.php?option=com_properties&view=localities&layout=form&task=edit&cid[]='.$id);
	
				break;
			case 'save':
	$this->setRedirect( 'index.php?option=com_properties&view=localities');
	
				break;	
			case 'save2new':
				$this->setRedirect(JRoute::_('index.php?option=com_properties&view=localities&layout=form&task=edit', false));
	$msg.='. '.JText::_('You can add new Locality');
				break;	
...
foreach($cids as $cid) {
			...
			$msg .= JText::_( 'Borrado localit : '.$cid );
			
}			
		}

		$this->setRedirect( 'index.php?option=com_properties&view=localities', $msg );
	}
