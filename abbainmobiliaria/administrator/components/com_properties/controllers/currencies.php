...
class PropertiesControllerCurrencies extends PropertiesController
{	 
...
function publish()
	...
	$this->publish	= ( $this->getTask() == 'publish' ? 1 : 0 );		
		...
		$query = 'UPDATE #__properties_currencies'
		...
		$link = 'index.php?option=com_properties&view=currencies';
	...
	$db			=& JFactory::getDBO();
	$row		=& JTable::getInstance('currencies','Table');
	$row->load( $uid );
	$row->move( $inc );			
...
$msg = 	JText::_('Saved').' ( '.$post['currencies'].' ) ';
				
				switch (JRequest::getCmd( 'task' ))
		{
			case 'apply':
	$this->setRedirect( 'index.php?option=com_properties&view=currencies&layout=form&task=edit&cid[]='.$id);
	
				break;
			case 'save':
	$this->setRedirect( 'index.php?option=com_properties&view=currencies');
	
				break;	
			case 'save2new':
				$this->setRedirect(JRoute::_('index.php?option=com_properties&view=currencies&layout=form&task=edit', false));
	$msg.='. '.JText::_('You can add new Currency');
				break;			
		}
...
foreach($cids as $cid) {
			...
			$msg .= JText::_( 'Borrado currency : '.$cid );
			
}			
		}

		$this->setRedirect( 'index.php?option=com_properties&view=currencies', $msg );
	}
