...
<form action="index.php?option=com_properties&amp;view=localities&amp;layout=multimodal&amp;tmpl=component&amp;object=<?php echo JRequest::getVar('object');?>&amp;value=<?php echo JRequest::getVar('value'); ?>" method="post" name="adminForm">
...
<table>
	<tr>
		<td align="left" width="100%">
			...
			<button onclick="document.getElementById('search').value='';document.getElementById('filter_multisid').value='';this.form.submit();"><?php echo JText::_( 'Reset' ); ?></button>
		</td>        

         <td nowrap="nowrap">
            <?php echo Filter::FilterMultiSid( $this->items[0],'state','locality' ); ?>
            </td>
            <td valign="top">
...
<?php } ?>
			<td>             
            <span class="editlinktip hasTip" title="<?php echo JText::_( 'Select Category' );?>::<?php echo $row->name; ?>">
			<a style="cursor: pointer;" onclick="window.parent.jSelectLocality('<?php echo $row->id; ?>', '<?php echo str_replace(array("'", "\""), array("\\'", ""),$row->name); ?>', '<?php echo JRequest::getVar('object'); ?>');">
		...
		?>
		 ...
		 function areChecked()
			...
			for(a==0;a<<?php echo $i;?>;++a)
			 ...
			 window.parent.jSelectLocality(saveValue, saveValue, '<?php echo JRequest::getVar('object'); ?>');
			}		 
         </script> 	
	</tbody>
    <tfoot>
