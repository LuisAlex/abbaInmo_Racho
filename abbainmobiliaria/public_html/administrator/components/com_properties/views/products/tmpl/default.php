...
<tbody>
	<?php
	if(isset($this->items[0]->id) and $this->items[0]->id)
	{
	$k = 0;
...
$link 		= JRoute::_( 'index.php?option='.$option.'&view='.$view.'&layout=form&task=edit&cid[]='. $row->id);	
		...
		$published 	= Helper::Published( $row, $i);
	...
	?>
		<tr class="<?php echo "row$k"; ?>">
			...
			{
			$img_path = JURI::root().'images/properties/images/thumbs/'.$row->id.'/'.$this->Images[$row->id][0]->name;?>
