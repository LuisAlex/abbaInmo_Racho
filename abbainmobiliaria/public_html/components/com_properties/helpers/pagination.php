...
class JPaginationPanel extends JObject{
	...
	function getPagesLinks()
	{
		...
		$chromePath = JPATH_COMPONENT.DS.'helpers'.DS.'pagination_panel.php';
	...
	function orderUpIcon($i, $condition = true, $task = 'orderup', $alt = 'Move Up', $enabled = true)
	{
		...
		if (($i > 0 || ($i + $this->limitstart > 0)) && $condition)
		{
			if($enabled) {
				...
				$html	.= '   <img src="components/com_properties/includes/img/uparrow.png" width="16" height="16" border="0" alt="'.$alt.'" />';
				$html	.= '</a>';
			} else {
				$html	= '<img src="components/com_properties/includes/img/uparrow0.png" width="16" height="16" border="0" alt="'.$alt.'" />';
			}
		}
	...
	function orderDownIcon($i, $n, $condition = true, $task = 'orderdown', $alt = 'Move Down', $enabled = true)
	{
		...
		if (($i < $n -1 || $i + $this->limitstart < $this->total - 1) && $condition)
		{
			if($enabled) {
				$html	= '<a href="#reorder" onclick="return listItemTask(\'cb'.$i.'\',\''.$task.'\')" title="'.$alt.'">';
				$html	.= '  <img src="components/com_properties/includes/img/downarrow.png" width="16" height="16" border="0" alt="'.$alt.'" />';
				$html	.= '</a>';
			} else {
				$html	= '<img src="components/com_properties/includes/img/downarrow0.png" width="16" height="16" border="0" alt="'.$alt.'" />';
			}
		}
	...
	function _buildDataObject()
	{
		...
		$data->all	= new JPaginationObjectPanel(JText::_('View All'));
		if (!$this->_viewall) {
			$data->all->base	= '0';
		...
		$data->start	= new JPaginationObjectPanel(JText::_('Start'));
		$data->previous	= new JPaginationObjectPanel(JText::_('Prev'));
		...
		$data->next	= new JPaginationObjectPanel(JText::_('Next'));
		$data->end	= new JPaginationObjectPanel(JText::_('End'));
		...
		for ($i = $this->get('pages.start'); $i <= $stop; $i ++)
		{
			...
			$data->pages[$i] = new JPaginationObjectPanel($i);
 ...
 * @since		1.5
 */
class JPaginationObjectPanel extends JObject
{
	var $text;
	var $base;
	var $link;
